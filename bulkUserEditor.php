 ...
 * Plugin URI: https://wordpress.org/plugins/bulk-user-editor/
...
if (! class_exists('BulkUserEditor')) {
    ...
    class BulkUserEditor
    {
        ...
        function bue_modify_gf_fields()
        {
            ...
            if (empty($gf_form_field_key_cond)) {
            ...
            bulkusereditor_log($changeResults);
...
<select id="bme_post_type" name="">
    ...
    <?php } ?>
		...
		function bulk_edit_fields_submenu_handler() {
			...
			echo '</fieldset>';
			echo '<fieldset style="'.$fieldSetStyle.'"><legend class="bue_legend">maximum entries to process</legend><input type="number" name="gf_max_entries"></fieldset>';
			...
			if (/*isset ( $val ) &&*/ isset ( $meta ) && (isset ( $role ) || isset ( $userID ))) {
				...
				foreach ( $blogusers as $user ) {
					...
					if ($newValue == null) {
					    ...
					    $msg = 'WARN::setting blank value for user ' . $user_id;
					    bulkusereditor_log ( $msg );
					}
